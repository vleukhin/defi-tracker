 ...
 *
 * ЧТО ЭТИМ СПИСКОМ ЧИТАЕТСЯ: свободные средства на кошельке — то, что лежит
 * на адресе и не участвует ни в залоге, ни в позициях (reader.ts). Залог
 * Aave v3 читается отдельно (chains/aave.ts) по своему списку резервов.
 *
 * ЗДЕСЬ ЛЕЖАТ UNDERLYING-АДРЕСА, И ЭТО ВАЖНО. Положить сюда aToken, vToken,
 * fToken или GM-токен значит посчитать одни и те же деньги дважды: залог,
 * долг и позиции читаются своими контурами. Сторож в allowlist.test.ts
 * падает, если такой адрес или символ здесь появится.
 *
