...
export interface SnapshotDto {
  ...
  debtUsd: number | null;
  /**
   * Залог по оракулу Aave на момент съема — тот же базис, что у debtUsd,
   * поэтому LTV точки = debtUsd / collateralUsd. Не путать с
   * items[].collateralUsd: там залог оценен по CoinGecko и разложен
   * по трем категориям портфеля. null = здоровье в тот день не читалось.
   */
  collateralUsd: number | null;
  /**
   * МИНИМАЛЬНЫЙ health factor по (кошелек, сеть): ликвидация приходит
   * к худшей позиции. null двузначен и различается по debtUsd — 0 означает
   * «долга нет» («∞»), null — «здоровье не читалось».
   */
  healthFactor: number | null;
  /**
   * Размещенные позиции (Фаза 5) на момент съема. totalUsd — это ТОЛЬКО
   * портфель; Активы точки = totalUsd + positionsUsd + freeBorrowedUsd.
  ...
  freeUsd: number | null;
  /**
   * Свободные ЗАЕМНЫЕ средства на кошельках на момент съема. В totalUsd
   * и freeUsd НЕ входят (те ведутся по собственным средствам), в Активы
   * точки входят целиком — иначе Долг вычитался бы полностью, а занятые
   * и еще не размещенные деньги в Активах не значились бы.
   * null = балансы на эту дату не читались (или точка снята до колонки).
   */
...
export interface PositionDto {
   ...
   * От нее считаются уровни действий −7 / −15 / −30 / −50 / −70 (docs/07 §5).
